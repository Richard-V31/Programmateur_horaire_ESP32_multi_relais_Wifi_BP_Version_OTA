...
//  (unique, sans espace, ex: "1. ..") et la broche GPIO. Pour en retirer un,
...
// index alignés avec programmateurs[]). Non utilisé si pinBP == -1.
...
.mode-btn.auto{
  ...
  /* 🚨 Texte sombre (au lieu du blanc hérité de .mode-btn) : le fond orange clair
     offrait un contraste trop faible avec du texte blanc (~1.6:1, sous le
...
.wifi-pct.good{color:var(--ok);}       /* >= 67% : bon signal */
.wifi-pct.mid{color:#fbbf24;}          /* 34-66% : signal moyen */
.wifi-pct.weak{color:#fb7185;}         /* < 34% : signal faible */
...
//  "1.xx") et des suffixes courts ("debut","fin","auto","etM"), on reste
...
// NB_PROGRAMMATEURS relais à chaque fois. 💾 Utile pour limiter l'usure de la
...
void setupOTA() {
  ...
  // Callbacks : affichent la progression sur l'écran OLED (si présent) et sur
  ...
  ArduinoOTA.onStart([]() {
    ...
    if (oledOK) {
      ...
      display.println("Ne pas eteindre !");
      display.display();
    }
  });
  ...
  ArduinoOTA.onEnd([]() {
    ...
    if (oledOK) {
      display.clearDisplay();
      display.setCursor(0, 0);
      display.println("Mise a jour OK !");
